...
var _hwTasks = JSON.parse(localStorage.getItem('aul_hw') || '[]');
function hwSave() {
...
function hwDtpShiftMonth(delta) {
    ...
    if (cal) {
        ...
        cal.getBoundingClientRect(); // reflow to restart — side effect read
...
function hwDtpSelectDay(y, m, d, _el) {
    hwDtp.year = y;
    hwDtp.month = m;
    hwDtp.day = d;
    ...
    if (grid) {
        ...
        all.forEach(btn => {
            ...
            btn.getBoundingClientRect(); // reflow to restart animation — side effect read
...
function hwDtpSyncTime() {
    ...
    if (hv && document.activeElement !== hv)
        hv.value = String(hwDtp.hour).padStart(2, '0');
    if (mv && document.activeElement !== mv)
        mv.value = String(hwDtp.min).padStart(2, '0');
}
...
function hwDtpHourInput(el) {
    ...
    if (el.value.length === 2 && !Number.isNaN(Number.parseInt(el.value, 10))) {
        const minVal = document.getElementById('hwDtpMinVal');
        if (minVal) {
            minVal.focus();
            minVal.select();
        }
...
function hwDtpTimeKey(e, field) {
    if (e.key === 'ArrowUp') {
        e.preventDefault();
        field === 'hour' ? hwDtpShiftHour(1) : hwDtpShiftMin(1);
    }
    if (e.key === 'ArrowDown') {
        e.preventDefault();
        field === 'hour' ? hwDtpShiftHour(-1) : hwDtpShiftMin(-1);
...
function hwDtpIsoValue() {
    if (hwDtp.year === null || hwDtp.month === null || hwDtp.day === null)
        return '';
...
function calculateDiffDays(dateStr, today) {
...
function createDueSoonBannerElement(urgentTask, urgentDiffDays) {
...
function hwSectionHeader(task, currentSection) {
    let label = 'Upcoming';
...
function hwStatusHtml(task, overdue, todayMidnight) {
    if (task.done)
        return `<span class="pill pill-done">Done</span>`;
    if (overdue.includes(task))
        return `<span class="pill pill-overdue">Overdue</span>`;
    if (task.date) {
        const diffDays = calculateDiffDays(task.date, todayMidnight);
        if (diffDays <= 5)
            return `<span class="pill pill-soon">Due soon</span>`;
    }
    return `<span class="pill pill-todo">To do</span>`;
}
function hwRowClass(task, overdue) {
    if (task.done)
        return 'done-row';
    else if (overdue.includes(task))
        return 'overdue-row';
    else
        return 'upcoming-row';
}
...
function hwExportPDF() {
    if (!_hwTasks.length) {
        showToast('Nothing to export', 'Add some tasks first');
        return;
    }
    const win = window.open('', '_blank');
    if (!win) {
        showToast('Popup blocked', 'Please allow popups for PDF export');
        return;
    }
...
function hwShare() {
    ...
    if (btn?.dataset.sharing)
        return; // block rapid re-clicks
...
document.getElementById('hwSubject')?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
        e.preventDefault();
        document.getElementById('hwDesc')?.focus();
    }
});
