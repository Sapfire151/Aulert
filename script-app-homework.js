...
function hwAdd() {
    ...
    try {
        hwSave();
        hwRender();
        if (document.getElementById('tb-cal')?.classList.contains('on'))
            renderCal();
...
function hwExportPDF() {
    ...
    const rows = allSorted.map(t => {
        ...
        if (t.done)
            sectionClass = 'done';
        else if (overdue.includes(t))
            sectionClass = 'overdue';
        else
        ...
        let statusHtml = '';
        if (t.done)
            statusHtml = `<span class="pill pill-done">Done</span>`;
        else if (overdue.includes(t))
            statusHtml = `<span class="pill pill-overdue">Overdue</span>`;
        else {
            // Check if due soon
            let dueSoon = false;
            if (t.date) {
