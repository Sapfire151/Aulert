...
function renderSidebar() {
    ...
    if (clsEl) {
        ...
        const addCls = (id, color, name, section, count) => {
            ...
            div.onkeydown = function (e) { if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                setCourseFilter(id, this);
            } };
...
const SETTING_LABELS = {
    stream: 'Stream posts', announcements: 'Announcements', assignments: 'Assignments',
...
function saved(label = 'Settings', value) {
    ...
    S.snackTimer = setTimeout(() => s.classList.remove('show'), 3000);
    saveSettings();
}
function saveSetting(key, val) {
    ...
    try {
        saveSettings();
        saved(SETTING_LABELS[key] || key, Boolean(val));
    }
    catch (e) {
...
async function gcalSyncAll() {
    ...
    if (errors)
        showToast('Calendar sync (partial)', msg + ' · ' + errors + ' failed');
    else
        showToast('Google Calendar synced ✓', msg);
    gcalRenderStatus();
}
async function gcalUnsyncAll() {
    ...
    for (const gcalId of Object.values(map)) {
        try {
            await gcalRequest('DELETE', '/calendars/primary/events/' + gcalId);
            removed++;
        }
        catch (e) {
            console.warn('Failed to delete gcal event:', e);
...
function gcalRenderStatus() {
    ...
    if (syncBtn)
        syncBtn.style.display = S.settings.gcalSync ? 'inline-flex' : 'none';
    if (unsyncBtn)
        unsyncBtn.style.display = (S.settings.gcalSync && count > 0) ? 'inline-flex' : 'none';
}
...
function renderSettings() {
    const m = S.settings;
    const set = (id, val) => { const e = document.getElementById(id); if (e)
        e.checked = !!val; };
...
async function discordAdd(form) {
    ...
    try {
        ...
        if (button) {
            ...
            button.textContent = 'Add destination';
        }
    }
}
...
async function discordRemove(webhookId) {
    if (!await showConfirmDialog('Remove destination?', 'This Discord webhook will stop receiving Classroom updates.', 'Remove'))
        return;
    try {
        if (S.token?.startsWith('preview_bypass')) {
...
async function discordDisconnect() {
    if (!await showConfirmDialog('Disconnect all Discord destinations?', 'This permanently removes the stored offline authorization and all webhooks.', 'Disconnect'))
        return;
    try {
        if (S.token?.startsWith('preview_bypass')) {
            discordConfig = { enabled: false, webhooks: [] };
