...
function loadAnalyticsIfAllowed() {
    ...
    let consent;
    try {
        consent = JSON.parse(localStorage.getItem('aul_cookie_consent_v1') || 'null');
    }
    catch {
        consent = null;
    }
...
function initCookieConsent() {
    ...
    banner.querySelectorAll('[data-cookie-choice]').forEach((button) => button.addEventListener('click', () => {
        ...
        try {
            localStorage.setItem('aul_cookie_consent_v1', JSON.stringify(selection));
        }
        catch (e) {
            console.warn('Functional storage blocked', e);
        }
...
function loadLastSeen() {
    try {
        return JSON.parse(localStorage.getItem('aul_lastseen') || '{}');
    }
    catch {
        return {};
    }
...
function doAuth() {
    if (CLIENT_ID.startsWith('YOUR_CLIENT_ID')) {
        ...
        if (p) {
            ...
            p.textContent = 'Please set your Google OAuth Client ID in the CONFIG at the top of the script.';
        }
        return;
    }
    if (!window.google?.accounts?.oauth2) {
...
async function fetchAllContent(initial = false, isIncremental = false) {
    ...
    if (initial && typeof renderClasses === 'function')
        renderClasses();
...
async function fetchCourse(course, lastSeen = {}, isIncremental = false) {
    ...
    if (subs.status === 'fulfilled' && subs.value.studentSubmissions) {
        subs.value.studentSubmissions.forEach(s => {
            if (s.state === 'TURNED_IN' || s.state === 'RETURNED')
                turnedInIds.add(s.courseWorkId);
    ...
    if (ann.status === 'fulfilled') {
        (ann.value.announcements || []).forEach(a => {
            ...
            if (!isNewer(effectiveTime, annKey) && isIncremental)
                return;
    ...
    if (mat.status === 'fulfilled') {
        (mat.value.courseWorkMaterial || []).forEach(m => {
            ...
            if (!isNewer(effectiveTime, matKey) && isIncremental)
                return;
            notifs.push({
                id: `mat-${m.id}`,
                type: 'material',
...
async function fetchGmailComments() {
    ...
    if (!S.settings.comments)
        return notifs; // User setting to disable if needed
    if (S.token.startsWith('preview_bypass')) {
        return [{
    ...
    try {
        ...
        const msgPromises = listData.messages.map(m => fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${m.id}?format=metadata&metadataHeaders=Subject&metadataHeaders=Date`, {
            headers: { Authorization: `Bearer ${S.token}` }
        }).then(r => r.json()));
...
function startPolling() {
    ...
    S.countdownTimer = setInterval(() => {
        ...
        if (el)
            el.textContent = secs > 0 ? 'Refreshing in ' + secs + 's' : 'Refreshing…';
    }, 1000);
}
/**
    ...
    document.querySelectorAll('[data-theme-toggle], .mob-top-bar button').forEach((toggle) => {
        ...
        if (el) {
            ...
            if (window.MorphIcons)
                window.MorphIcons.set(el, iconName);
        }
...
function goTab(name) {
    ...
    ['feed', 'cal', 'hw', 'set', 'com'].forEach(t => {
        ...
        if (panel)
            panel.classList.toggle('show', t === name);
        if (tab)
            tab.classList.toggle('on', t === name);
        if (mobTab)
            mobTab.classList.toggle('on', t === name);
    });
...
function showToast(title, msg, type = 'default') {
    ...
    if (type !== 'default' && TYPE_META.has(type))
        color = TYPE_META.get(type).color;
    t.innerHTML = `
  ...
  `;
    ...
    t.addEventListener('focusout', () => { if (!t.contains(document.activeElement))
        scheduleDismiss(); });
...
(function () {
    function setCalDate() {
        ...
        if (el)
            el.textContent = String(new Date().getDate());
    }
    if (document.readyState === 'loading')
        document.addEventListener('DOMContentLoaded', setCalDate);
    else
        setCalDate();
})();
let _pendingFiles = [];
function showSubmitConfirm() {
    ...
    if (dropZoneText)
        dropZoneText.textContent = 'Click to browse or drag files here';
    if (fileList)
        fileList.innerHTML = '';
    if (linkInput)
        linkInput.value = '';
...
function addPendingFile(file) {
    if (_pendingFiles.some(f => f.name === file.name && f.size === file.size))
        return;
...
function renderFileList() {
    ...
    if (dropZoneText)
        dropZoneText.textContent = _pendingFiles.length + ' file' + (_pendingFiles.length > 1 ? 's' : '') + ' selected \u2014 click to add more';
    if (dropZone) {
