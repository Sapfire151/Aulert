...
function renderDl(day, direction = 0) {
  ...
  if (!combined.length) { el.innerHTML = `<div class="empty-s empty-s-compact"><h3>No deadlines${day ? ' on this day' : ''}</h3><p>${day ? 'Choose another date or add a homework task to plan ahead.' : 'Upcoming due dates and homework tasks will appear here.'}</p></div>`; return; }
