...
function renderNotifPanel() {
  ...
  cnt.textContent = String(unread); cnt.style.display = unread ? '' : 'none';
  ...
  const typeIcon = (type: string, color: string) =>
    `<span class="morph-icon" data-morph-icon data-icon="${typeIconName[type] || 'circle'}" data-size="13" data-color="${color || 'currentColor'}"></span>`;
  ...
  if (np && window.MorphIcons) window.MorphIcons.mount(np);
}
...
function renderFeed() {
  ...
  Object.keys(cnt).forEach(k => {
    const el = document.getElementById('cnt-'+k); if (el) el.textContent = String(cnt[k]);
  });
  // Deadline count
  const nowDay2 = new Date(); nowDay2.setHours(0,0,0,0);
  const dlCntEl = document.getElementById('cnt-deadline');
  if (dlCntEl) dlCntEl.textContent = String(S.deadlines.filter(d => d.date >= nowDay2).length);

  const unread = S.notifs.filter(n => !n.read).length;
  const du = document.getElementById('dashUnreadNum');
  if (du) du.textContent = String(unread);
