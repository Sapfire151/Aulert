...
function hwAdd() {
  ...
  const priorityEl = document.getElementById('hwPriority') as HTMLInputElement | null; // hidden input
  const tagsEl     = document.getElementById('hwTags') as HTMLInputElement | null;
...
function hwSetFilter(filter: string) {
  hwActiveFilter = filter;
  hwRender();
}

function hwToggleSortDropdown(e?: Event) {
...
const HW_SORT_LABELS: Record<string, string> = {
  'priority-due': 'Priority → Due Date',
  'due-priority': 'Due Date → Priority',
};

function hwSelectSortMode(mode: string, label?: string) {
  hwSortMode = mode;
  const text = label || HW_SORT_LABELS[mode] || mode;
  const labelEl = document.getElementById('hwSortLabel');
  if (labelEl) labelEl.textContent = text;

  const hidden = document.getElementById('hwSortSelect') as HTMLInputElement | null;
  if (hidden) hidden.value = mode;

  const menu = document.getElementById('hwSortMenu');
  if (menu) {
    menu.querySelectorAll<HTMLButtonElement>('.hw-dropdown-item').forEach(item => {
...
function hwSetSortMode(mode: string) {
 ...
 */
function hwSelectPriority(priority: string, btn?: HTMLElement) {
  // If triggered by a user click (btn passed), lock out auto-suggest
  if (btn) _hwPriorityAutoSet = false;
  // Deactivate all pills
  const group = document.querySelector('.hw-priority-group');
  if (group) {
    group.querySelectorAll<HTMLButtonElement>('.hw-priority-btn').forEach(b => {
      b.classList.remove('active');
      b.setAttribute('aria-pressed', 'false');
    });
    // Activate the matching pill
    const target = btn || group.querySelector<HTMLButtonElement>(`[data-priority="${priority}"]`);
    if (target) {
      target.classList.add('active');
      target.setAttribute('aria-pressed', 'true');
    }
  }
  const hidden = document.getElementById('hwPriority') as HTMLInputElement | null;
  if (hidden) hidden.value = priority;
}

/** Keyword → priority auto-suggest map */
const HW_PRIORITY_KEYWORDS: Record<string, string> = {
  exam: 'urgent',
  test: 'urgent',
  midterm: 'urgent',
  final: 'urgent',
...
function hwAutoSuggestPriority(value: string) {
  if (!_hwPriorityAutoSet) return; // user already chose manually
  const lower = value.toLowerCase();
  let suggested = 'normal';
  for (const [kw, pri] of Object.entries(HW_PRIORITY_KEYWORDS)) {
    if (lower.includes(kw)) { suggested = pri; break; }
  }
  const hidden = document.getElementById('hwPriority') as HTMLInputElement | null;
...
function createHwCardElement(task: any, today: Date, nowMs: number): HTMLElement {
  ...
  const tagsHtml = Array.isArray(task.tags) && task.tags.length
    ? task.tags.map((t: string) => `<span class="hw-tag">${escHtml(t)}</span>`).join('')
    : '';
...
function hwRenderFilterBar() {
  ...
  const priorityCounts: Record<string, number> = { urgent: 0, high: 0, low: 0 };
...
let _hwFirebaseDb: any = null;
let _hwSyncInitialized = false;

async function hwInitSync() {
  if (_hwSyncInitialized || !window.firebase || !S.token) return;
  ...
  try {
    ...
    tasksRef.on('value', (snapshot: any) => {
      const remoteData = snapshot.val();
      if (!remoteData) return;

      const remoteTasks: any[] = Array.isArray(remoteData)
        ? remoteData
...
function hwDeleteSyncTask(id: any) {
  if (_hwFirebaseDb && S.user?.id) {
    try {
